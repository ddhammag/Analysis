// -*- C++ -*-
//
// Package:    bRegression/bRegression
// Class:      bRegression
// 
/**\class bRegression bRegression.cc bRegression/bRegression/plugins/bRegression.cc
...
//         Created:  Wed, 25 Nov 2020 08:55:27 GMT
//
//
...
class bRegression : public edm::stream::EDProducer<> {
   ...
   public:
      bRegression(const edm::ParameterSet&);
      ~bRegression();
...
bRegression::bRegression(const edm::ParameterSet& iConfig):
    ...
    y_std_(iConfig.getUntrackedParameter<double>("y_std"))
{
   jetToken_= consumes<View<pat::Jet> >(inputTagJets_);
	...
	produces<edm::ValueMap<float>>("bRegression").setBranchAlias( "bRegression");
    produces<edm::ValueMap<float>>("bRegressionResolution").setBranchAlias( "bRegressionResolution");
	produces<edm::ValueMap<float>>("rawP4").setBranchAlias( "rawP4");
    produces<edm::ValueMap<float>>("jetPt").setBranchAlias( "jetPt");
...
void
bRegression::produce(edm::Event &evt, const edm::EventSetup & )
{
   InitJet();
    // input jets
    ...
    std::vector<float> *RAWP4 = new std::vector<float>();
	...
	for( unsigned int i = 0 ; i < jets->size() ; i++ ) {
        ...
        Jet_leptonPtRel = lepPtRel; 		//********** Jet_leptonPtRel = std::max(float(0.),fjet.userFloat("softLepPtRel"));
        Jet_leptonDeltaR = lepDeltaR; 		//********** Jet_leptonDeltaR = std::max(float(0.),fjet.userFloat("softLepDr"));
        Jet_leptonPtRelInv = lepPtRelInv;	//********** Jet_leptonPtRelInv = fjet.userFloat("softLepPtRelInv");
...
void bRegression::SetNNVectorVar(){
...
std::vector<float> bRegression::EvaluateNN(){
...
template <typename T,typename J> void bRegression::putInEvent(std::string name, const edm::Handle< J >& jets, std::vector<T>* product, edm::Event& iEvent){
